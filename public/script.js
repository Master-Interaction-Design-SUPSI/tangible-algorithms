...
ws.onmessage = function (event) {
  console.log("Messaggio dal server:\n", event.data);
};
...
function switchState(newState) {
  ...
  // Controlla se la connessione WebSocket è aperta prima di inviare il messaggio
  if (ws.readyState === WebSocket.OPEN) {
    // Prepara il messaggio come stringa JSON
    ...
    ws.send(message);
    console.log("Stato inviato al server:", message);
  } else {
    console.log("WebSocket non è connesso.");
  }
...
function showImages() {
  ...
  // Quando il timer di 5 secondi scade, chiama switchState(state.STATE_IMAGE_PROPERTIES)
  setTimeout(function () {
...
function showCaptionAndButtons() {
  ...
  // Aggiunta del timer di fallback
  clearTimeout(window.fallbackTimer); // Pulisci un precedente timer se esistente

  window.fallbackTimer = setTimeout(() => {
    console.log("Fallback: Nessun pulsante premuto entro 10 secondi.");
    proceedToNextScene(); // Procedi con la logica per andare alla scena successiva
  }, 10000); // Imposta il timer per 10 secondi
}

// Funzione per cancellare il timer e gestire il passaggio alla scena successiva
function proceedToNextScene() {
  // Qui, aggiungi la logica per passare alla scena successiva
  clearTimeout(window.fallbackTimer); // Pulisci il timer di fallback
